...
class App extends Component {
  ...
  getMatchingElements (term, numeric = false, attr) {
    let results = [];
    if (attr === "name") {
      term = term.replace(/\s/g, "");
      this.state.atoms.forEach(element => {
        if(element[attr].includes(term)) results.push(element.symbol);
      });
    } else {
      this.state.atoms.forEach(element => {
        const data = numeric? element[attr] : element[attr].toLowerCase()
  ...
  handleQuery = (query) => {
      ...
      if ((query.length<3) && (query.match(/[a-z]{1,2}\s?$/i))) { 
        results = this.getMatchingElements(query, false, "symbol");
      } else if(query.match(/\d{1,3}\s?$/i)) {
        results = this.getMatchingElements(parseInt(query, 10), true, "atomic_number");
      } else if (query.match(/non[-\s]?metals?\s?$/i)) {
        results = this.getMatchingElements("non-metal", false, "primary_class");
      } else if (query.match(/noble[-\s]?gas(es)?\s?$/i)) {
        results = this.getMatchingElements("noble-gas", false, "primary_class");
      } else if (query.match(/alkali[-\s]?metals?\s?$/i)) {
        results = this.getMatchingElements("alkali-metal", false, "primary_class");
      } else if (query.match(/alkaline[-\s]?earth[-\s]?metals?\s?$/i)) {
        results = this.getMatchingElements("alkaline-earth-metal", false, "primary_class");
      } else if (query.match(/metaux[-\s]?pauvres?\s?$/i)) {
        results = this.getMatchingElements("metaux_pauvres", false, "primary_class");
      } else if (query.match(/metalloids?\s?$/i)) {
        results = this.getMatchingElements("metalloid", false, "primary_class");
      } else if (query.match(/transition[-\s]?metals?\s?$/i)) {
        results = this.getMatchingElements("transition-metal", false, "primary_class");
      } else if (query.match(/lanthanoids?\s?$/i) || query.match(/lanthanides?\s?$/i)) {
        results = this.getMatchingElements("lanthanoid", false, "primary_class");
      } else if (query.match(/actinoids?\s?$/i) || query.match(/actinides?\s?$/i)) {
        results = this.getMatchingElements("actinoid", false, "primary_class");
      } else if (query.match(/unknowns?\s?$/i)) {
        results = this.getMatchingElements("unknown", false, "primary_class");
      } else {
