...
json_file = '../src/components/atoms.json'

with open(json_file) as json_data:
    data = json.load(json_data)

element_list = data['atoms']

for atom in element_list:
    print("import {} from './pages/elements/{}';".format(atom["name"].capitalize(), atom["name"].capitalize()))
    f = open("{}.js".format(atom["name"].capitalize()), "w")
    f.write('''
...
class {nameCap} extends Component {{

    state = {{
      atom: atoms.atoms[{digitalAtomicNo}],
      units: "celsius",
      }};
      ...
      render() {{
        return(
            <>
                ...
                <div id='content'>
                <h1>{nameCap} ({{this.state.atom.symbol}})</h1>  
...
export default {nameCap};
    '''.format(nameCap=atom["name"].capitalize(), atomicNo=atom["atomic_number"], digitalAtomicNo=atom["atomic_number"]-1))
    f.close()
